...
void sliceFitM4bLoop(float m4bStart = 400) 
{
    ...
    //for (float m4bSlice=0;m4bSlice<350;m4bSlice+=50) {
        float m4bSlice = m4bStart;
        float minm4b = m4bSlice;
        float maxm4b = m4bSlice+100;
        string pdfName = Form("sliceLoopM4bFit_hPt%dto%d.pdf",(int)minm4b,(int)maxm4b);
        ...
        //  Draw histogram 
        //string sCut = Form("isTag&&(hPt<%f)&&(hPt>%f)&&(Mh<136)&&(Mh>114)&MZp>400",maxm4b,minm4b);
        string sbCut = Form("isTag&&(hPt<%f)&&(hPt>%f)&&(Mh>136||Mh<114)&MZp>400",maxm4b,minm4b);
        t1->Draw("Mh>>h_sb(25,100,150)",sbCut.data(),"e");
        t1->Draw(Form("hPt>>h_hptPass(35,%f,%f)",minm4b-10,maxm4b+10),sbCut.data(),"e");
        ...
        //string sfCut = Form("isAntiTag&&(MA0<%f)&&(MA0>%f)&&(Mh<136)&&(Mh>114)&MZp>400",maxm4b,minm4b);
        //string sfCut = Form("isAntiTag&&(hPt<%f)&&(hPt>%f)&&MZp>400",maxm4b,minm4b);
        //string sfCut_nosig = Form("isAntiTag&&(hPt<%f)&&(hPt>%f)&&MZp>400&&(Mh>136||Mh<114)",maxm4b,minm4b);
        ...
        t1->Draw(Form("hPt>>h_hptFail(35,%f,%f)",minm4b-10,maxm4b+10),sfCut_nosig.data(),"e");
        ...
        for (int k=0;k<t1->GetEntries();k++) {
            ...
            if (isTag&&0) { 
            	if (m4b>maxm4b||m4b<minm4b) continue; //[200,300]
            ...
            if (isHfail) {
                ...
                mh_loop = (*v1+*v2).M();
                if (m4b_loop>maxm4b||m4b_loop<minm4b) continue;
        ...
        h_Rpf->SetTitle(Form("h_Rpf_hPt%d-%d",(int)minm4b,(int)maxm4b));
        //h_srRpf->Sumw2();
        h_Rpf->Sumw2();
        //h_srRpf->Divide(h_cr);
        h_Rpf->Divide(h_cr);
        ...
        h_sb->SetTitle(Form("h_sideband_hpt%d-%d",(int)minm4b,(int)maxm4b));
    ...
    for (int i=0;i<2;i++) {
        ...
        string namep = Form("h_reduceMass_pass_hPt%dto%d_sig%d",hPtp,(int)maxm4b,i);
        string nameAl = Form("h_reduceMass_Alphabet_hPt%dto%d_sig%d",hPtp,(int)maxm4b,i);
        string name4bp = Form("h_4bMass_pass_hPt%dto%d_sig%d",hPtp,(int)maxm4b,i);
        string name4bAl = Form("h_4bMass_Alphabet_hPt%dto%d_sig%d",hPtp,(int)maxm4b,i);
    ...
    */
    ...
    for (int i=0;i<t1->GetEntries();i++) {
        ...
        if (isTag) {
            if (m4b<400) continue;
            if (m4b>maxm4b||m4b<minm4b) continue;
        	if (mh>150||mh<100) continue; // Mh boundary
        	if (mh>114&&mh<136) continue;
    ...
    leg->SetHeader(Form("hPt%d-%d",(int)minm4b,(int)maxm4b),"C");
    ...
    //h_reduceMass_pass[0]->SetTitle(Form("h_reduceMass_hPt%d-%d",(int)minm4b,(int)maxm4b));
    ...
    //h_4bMass_pass[0]->SetTitle(Form("h_4bMass_hPt%d-%d",(int)minm4b,(int)maxm4b));
